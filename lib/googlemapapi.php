...
/*
/**
 * Customization where done to use Cms database abstract layer instead of
 * Pear to better integrate with deficiencies application.
/*
...
class GoogleMapAPI {
    ...
    /**
     * DataSource for geocode caching.
     *
  ...
  function addWaypointByAddress($address,$stopover = TRUE)
  {
	...
	*/
    ...
    function getCache($address) {
        ...
        $_db = new \Cms\DBAL\DataBase($this->dsn);
        
        $select = new \Cms\DBAL\Query\Select($this->_db_cache_table);
        $select->Select('lon')->Select('lat')->WhereEqual('address', $address, Cms\Enumerations\FieldType::TEXT);
    ...
    function putCache($address, $lon, $lat) {
        ...
        $insert = new \Cms\DBAL\Query\Insert($this->_db_cache_table);
        $insert->Insert('address', $address, Cms\Enumerations\FieldType::TEXT)
            ->Insert('lon', $lon, Cms\Enumerations\FieldType::REAL)
            ->Insert('lat', $lat, Cms\Enumerations\FieldType::REAL)
        ;
    ...
    function reverseGeoGetAddress($lon, $lat)
    {
        switch($this->lookup_service) {
            case 'GOOGLE':
                $_url = sprintf('http://%s/maps/api/geocode/json?sensor=%s&latlng=%s,%s',$this->lookup_server['GOOGLE'], $this->mobile==true?"true":"false", $lat, $lon);
                $_result = false;
                if($_result = $this->fetchURL($_url)) {
                    $_result_parts = json_decode($_result);
                    if($_result_parts->status!="OK"){
