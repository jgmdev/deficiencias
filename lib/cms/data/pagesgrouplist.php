...
{   
    ...
    /**
     * Get the array of groups and pages which the current user has access to
     * by reading the permissions property of each page.
     */
    public function GetPermittedGroups()
    {
        ...
        foreach($this->groups as $group_name=>$pages)
        {
            ...
            foreach($pages as $page_uri=>$page)
            {
                ...
                {
                    if(Authentication::GetGroup()->HasPermission($permission))
                    {
                        $pages_copy[$page_uri] = $page;
                    }
                }
            }
            
            if(count($pages_copy) > 0)
                $groups_copy[$group_name] = $pages_copy;
        }
